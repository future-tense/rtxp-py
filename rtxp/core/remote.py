...
class Remote(object):
	...
	def create_find_path(
		...
		return self.__command('path_find', local)

	def close_find_path(self, async=None):
		""" Closes a find_path session """

		subcommand = 'close'
		return self.__command('path_find', locals())
	...
	def get_static_path(
		...
		return self.__command('ripple_path_find', locals())
	...
	def cancel_offer(self, secret, account, offer_sequence, async=None):
		...
		if not account:
			account = self.signer.account_from_seed(secret)

		def on_success(res):
			seq, fee = res
			tx_json = self.transaction.offer_cancel(
				account,
				offer_sequence,
				seq,
				fee
	...
	def create_offer(self, secret, account, taker_gets, taker_pays,
		...
		def on_success(res):
			seq, fee = res
			tx_json = self.transaction.offer_create(
				account,
				taker_gets,
				taker_pays,
				seq,
	...
	def send_payment(self, secret, account, destination, amount,
		...
		def on_success(res):
			seq, fee = res
			tx_json = self.transaction.payment(
				account,
				destination,
				amount,
				seq,
	...
	def set_options(self, secret, account, async=None, **kwargs):
		...
		def on_success(res):
			seq, fee = res
			tx_json = self.transaction.account_set(
				account,
				seq,
				fee,
				**kwargs
	...
	def set_regular_key(self, secret, account, regular_key, async=None):
		...
		def on_success(res):
			seq, fee = res
			tx_json = self.transaction.set_regular_key(
				account,
				regular_key,
				seq,
				fee
	...
	def set_trust(self, secret, account, amount, async=None, **kwargs):
		...
		def on_success(res):
			seq, fee = res
			tx_json = self.transaction.trust_set(
				account,
				amount,
				seq,
				fee,
